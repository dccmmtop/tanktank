module Users
  module UserActions
    ...
    included do
      ...
      before_action :only_user!, only: [:topics, :replies, :favorites, :events, :notes,
    ...
    def events
      @events = @user.events.recent.paginate(page: params[:page], per_page: 40)
      fresh_when([@events])
    end
    ...
    def add_friend
      if params[:uid] && params[:event]
        if params[:uid].to_i != current_user.id
          user = User.find(params[:uid])
          if user
            user.friendships.create(friend_id: current_user.id, event: params[:event])
          end
        end
      end 
    end
