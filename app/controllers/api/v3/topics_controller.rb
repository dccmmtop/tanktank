  ...
  module V3
    class TopicsController < Api::V3::ApplicationController
      ...
      def create
        ...
        #body = HtmlHelper::Hcm.to_markdown(params[:body])
        body = ReverseMarkdown.convert(params[:body])
        if (params[:tags])
          tags = params[:tags]
          Keyword.set_interest(current_user, tags)
        end
        @topic = current_user.topics.new(title: params[:title], body: body, author: params[:author], source: params[:source], tags: params[:tags], source_date: params[:source_date])
