...
class ApplicationController < ActionController::Base
  ...
  # Judge Wechat share link? 
  def share_link
    if (session[:topic_id] && session[:user_id] && current_user)
      Rails.logger.info("==========************redirect to application   Followed_id is #{session[:user_id]}, current user is #{current_user.id}")
      if current_user.id != session[:user_id].to_i
        case session[:action].to_s
        when 'Topic'
          if Share.find_by(user_id: session[:user_id], friend_id:current_user.id, event: session[:action].to_s, event_id: session[:topic_id].to_i).nil? 
            User.find(session[:user_id]).shares.create(friend_id: current_user.id, event: session[:action].to_s, event_id: session[:topic_id].to_i)
          end
          redirect_to topic_url(Topic.find(session[:topic_id]), uid: current_user.id)
        when 'Passage'
          if Share.find_by(user_id: session[:user_id], friend_id:current_user.id, event: session[:action].to_s, event_id: session[:topic_id].to_i).nil? 
            if Passage.find(session[:topic_id]).status == Passage.audit_statuses[:audit_success]
              User.find(session[:user_id]).shares.create(friend_id: current_user.id, event: session[:action].to_s, event_id: session[:topic_id].to_i)
            end  
          end
          redirect_to wechat_passage_url(Passage.find(session[:topic_id]), uid: current_user.id)
        else
    ...
    if(session[:event])
      redirect_to wechat_friend_url(event:session[:event], uid: session[:user_id], request_id: session[:request_id], access: session[:access], token: session[:token])
    end
