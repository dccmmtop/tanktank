class TopicsController < ApplicationController
  ...
  def node
    ...
    @topics = @topics.paginate(page: params[:page], per_page: 25)
  ...
  def create
    ...
    if @topic.recommendation && @topic.source.blank? 
      @topic.source = "产学联原创"
    end
    if @topic.save
      if params[:message]
        link = "http://#{wechat_topic_path(@topic, uid: current_user.id)}"
        Message.send_single_message(@topic, current_user.id, link)
      end
    end
    @topic
  end
  ...
  def update
    ...
    @topic.summary = topic_params[:summary]
    @topic.cover = topic_params[:cover]
    @topic.recommendation = topic_params[:recommendation]
    if @topic.recommendation && topic_params[:source].blank? 
      @topic.source = "产学联原创"
    else
      @topic.source = topic_params[:source]
    end
    @topic.tags = topic_params[:tags]
    Keyword.set_interest(current_user, @topic.tags)
    @topic.save
  end
  ...
  def topic_params
    params.require(:topic).permit(:title, :body, :node_id, :team_id, :cover, :summary, :recommendation, :source, :tags)
  end
