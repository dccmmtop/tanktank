...
class Topic < ApplicationRecord
  ...
  validates :summary, length: { minimum: 10}, allow_blank: true 
  ...
  scope :today_recommendations, -> { where("recommendation = true and created_at >= ?", Time.zone.now.beginning_of_day) }
...
#     indexes :body, term_vector: :yes
  ...
  def self.latest_two(current_user)#从圈子里面找出最新的两篇文章
    current_user.all_topics[0...2]
  end
