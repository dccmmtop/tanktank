...
class User < ApplicationRecord
  ...
  has_many :nodes,through: :subscriptions
  has_many :invites, dependent: :destroy

# attr_accessor :password_confirmation

  ACCESSABLE_ATTRS = [:name, :email_public, :location, :company, :bio, :website, :github, :twitter,
                      :tagline, :avatar, :by, :current_password, :password,
                      :_rucaptcha, :title, :university, :major, :degree, :realname, :telephone, :speciality]
  ...
  # validates :company, length: { minimum: 2 }, presence: true
  ...
  def display_name
    return realname if !realname.blank?
...
#   created_at > 1.week.ago
  ...
  def node_ids
    ids=[]
    Subscription.where("user_id=? and apply_status=?",self.id,Subscription.applications[:passed]).each do |subscription|
      ids << subscription[:node_id]
    end
    ids
  end
  ...
  def all_topics
    ...
    Topic.all.each do |topic|
      if node_ids.include?(topic.node_id)#显示出当前用户关注的所有圈子的文章
        puts topic.title
        @topics << topic
      end
    end
