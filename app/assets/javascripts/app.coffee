...
AppView = Backbone.View.extend
  ...
  events:
    ...
    "click a.button-scan-user": "scanUser"
    "click a.button-scan-wechat-user": "scanWechatUser"
    "click #check_nickname":"getNickName"
  ...
  initialize: ->
    ...
    if $('body').data('controller-name') in ['topics', 'replies', 'wechat/topics']
    ...
    if $('body').data('controller-name') in ['events']
      window._eventView = new EventView({parentView: @})
  ...
  scanUser: (e) ->
    btn = $(e.currentTarget)
    userId = btn.data("id")
    event_text = $("#textEvent").val()
    url = "?uid=#{userId}"+"&event=" + encodeURIComponent(event_text)
  ...
  scanWechatUser: (e) ->
    ...
    event_text = $("#textEvent").val()
    url = "?uid=#{userId}"+"&event=" + encodeURIComponent(event_text)
    target = location.href.replace("acquaint","friend") + url
    # alert(target + )
    window.SocialShareWeChatButton.qrcode({
      url: target,
      header: '相识',
  ...
  getNickName: (e) ->
    ...
    if $("#check_nickname").is(':checked')
      $('#user_realname').val(nickname)
    else
      $('#user_realname').val('')
